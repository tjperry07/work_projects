...
2. IMPORTANT: If four attempts fail or progress stalls, stop acting and report what you observed, what blocked progress, and the most likely next step. **Exception:** `/docs-agent-guide-author` and `/docs-agent-guide-verify` use the extended **Path discovery** budget in [`.cursor/reference/guide-quality-rubric.md`](../guide-quality-rubric.md) before stopping on a missing nav path.
...
When waiting for page changes, prefer short CDP polling loops with Runtime.evaluate, DOM queries, Page lifecycle signals, or browser_snapshot checks rather than a single long wait. **`browser_wait_for` may be unavailable on cursor-ide-browser`**—use **`sleep 2–4`** plus fresh **`browser_snapshot`** until URL/title/refs match the destination (a generic **Coalesce Catalog** title usually means still loading).
